...
export class CartService {
  ...
  async addItem(userId: number, data: AddCartItemDto): Promise<CartItem> {
    ...
    const cartItem = await this.cartItemRepository.findItem(cart.id, product.id)
    if (!cartItem) {
  ...
  async updateQuantity(userId: number, productId: number, data: UpdateCartItemDto): Promise<CartItem> {
    if (!productId || productId <= 0) {
      throw new BadRequestException('Некорректный id товара')
    }
  ...
  private async findCartItemOrThrow(cartId: number, productId: number): Promise<CartItem> {
  ...
  private validateStock(requestedQuantity: number, stock: number) {
