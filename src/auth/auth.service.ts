...
export class AuthService {
  constructor(private readonly userService: UsersService, private readonly sessionService: SessionsService, private readonly jwtService: JwtService) { }
  ...
  async login(data: LoginUserDto) {
    ...
    const validPassword = await this.compareHash(data.password, user.passwordHash)
  ...
  async refresh(data: RefreshCookie){
    ...
    const validRefreshToken = await this.compareHash(refreshToken, session.refreshHash)
  ...
  async logout(data: RefreshCookie){
    const refreshToken = data.cookies.refreshToken
    if(!refreshToken){
      return
    }
  ...
  async logoutAll(data: RefreshCookie){
    const refreshToken = data.cookies.refreshToken
    if(!refreshToken){
      return
    }
